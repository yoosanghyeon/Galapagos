...
public class FeedWriteActivity extends AppBaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.feed_wirte);
        ...
        // 카테고리 레이아웃(상단 색깔)
        writeSelectedLayout = (RelativeLayout) findViewById(R.id.write_selected_layout);
        // 키워드(제목)
        editWriteTag = (EditText) findViewById(R.id.edit_write_tag);
        // 게시물 카테고리 선택 버튼()
        writeCatSelected = (ImageButton) findViewById(R.id.write_cat_select);
        // 게시물 작성시 자신의 사진
        imageWriteProfile = (CircleImageView) findViewById(R.id.image_write_profile);
        // 유저 이름
        textWriteUserName = (TextView) findViewById(R.id.text_write_user_name);
        // 현재 유저 위치 또는 지역 검색하는 위치를 보여주는 텍스트뷰
        textWriteUserLocation = (TextView) findViewById(R.id.text_write_user_location);
        // 유저가 작성한 text
        editFeedWriteContent = (EditText) findViewById(R.id.edit_feed_write_content);
        // Visialbe 해줄 레이아웃
        picUploadLayout = (LinearLayout) findViewById(R.id.pic_write_upload_layout);
        // 사진의 이미지뷰
        picUploadViewOne = (ImageView) findViewById(R.id.pic_write_upload_view1);
        // 지역등록(현재 위치가 default)
        btnWriteLocation = (ImageButton) findViewById(R.id.btn_wirte_location);
        // 사진 첨부 버튼
        btnWritePic = (ImageButton) findViewById(R.id.btn_write_pic);
        // 게시물 익명버튼
        btnWriteAnonymity = (ImageButton) findViewById(R.id.btn_wirte_anonymity);
        // 게시물 전송버튼
        btnFeedSendConfirm = (ImageButton) findViewById(R.id.btn_feed_send_confirm);
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_wirte_anonymity:
                ...
                break;
            case R.id.btn_feed_send_confirm:
                //게시글 등록 버튼 클릭시
                ...
                if (fwValueObject.feedWriteContent == null || fwValueObject.feedWriteContent.length() <= 0) {
                    ...
                    new AsyncFeedWriteBoard(this).execute(fwValueObject);
                    finish();
    ...
    //게시글 입력 및 parsing
    private class AsyncFeedWriteBoard extends AsyncTask<FeedWriteValue, Integer, String> {
        private Context mContext;

        final MediaType pngType = MediaType.parse("image/*");

        public AsyncFeedWriteBoard(Context context) {
            mContext = context;
        }
        ...
        protected String doInBackground(FeedWriteValue... feedWriteValues) {
            ...
            Log.e("feedWriteValues", feedWriteValues[0].toString());
            ...
            try {
                ...
                Log.e("boardProfile", PropertyManager.getInstance().getPicUri());
                multipartBody = new MultipartBody.Builder()
                        ...
                        .addFormDataPart("boardLat", new String().valueOf(feedWriteValues[0].feedWriteLat))
                        .addFormDataPart("boardLong", new String().valueOf(feedWriteValues[0].feedWriteLong))
                        .addFormDataPart("boardLocationSub", feedWriteValues[0].feedLocationName);

                Log.e("feedWriteAnom", "" + feedWriteValues[0].feedWriteisAnomynity);
                ...
                Request request = new Request.Builder()
                        .url(NetworkDefineConstant.SERVER_URL_MYWRITE_BOARD)
                        .post(fileUploadBody)
                        .build();
